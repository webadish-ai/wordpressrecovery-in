...
const UA = 'WordPressRecovery.in Security Scanner (+https://www.wordpressrecovery.in)';
...
export async function runScan(rawUrl: string, opts: { safeBrowsingKey?: string } = {}): Promise<ScanResult> {
  ...
  if (home.status === 403 || home.status === 503) {
    findings.push({
      category: 'blacklist',
      severity: 'warning',
      title: `Site returned HTTP ${home.status}`,
      detail: `The homepage responded with status ${home.status}, which often means the hosting account is suspended or the site is in a blocked/maintenance state — a common consequence of a malware suspension.`,
      recommendation: 'If your host suspended the account for malware, the site must be cleaned to their requirements before reactivation.',
    });
  }
  ...
  // --- Malware / injection signals in the HTML ---
  ...
  if (!findings.some((f) => f.category === 'malware')) {
